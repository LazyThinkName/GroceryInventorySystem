			...
			else
			{
				...
				if (searchBy != 6)
				{
					...
					if (searchBy == 1)
					{
						...
						cin.ignore(1);
						getline(cin, id);
						manage.searchID(id, searchBy, searchResult, foundStatus);
					}
					if (searchBy == 2 || searchBy == 5)//search by item name or item id
					{
						string search;
						if (searchBy == 2)
						{
					...
					else if (searchBy == 3)//search by price
					{
						...
						cin >> min;
						while (cin.fail() || min < 0) // check the validation of the input
						{
							cin.clear();
							cin.ignore(1, '\n');
							cout << "Invallid price\n"
								<< "Please enter your price again : ";
							cin >> min;
							cout << endl;
						}
						cout << "Enter maximum price: RM ";
						cin >> max;
						while (cin.fail() || max < 0) // check the validation of the input
						{
							cin.clear();
							cin.ignore(1, '\n');
							cout << "Invallid price\n"
